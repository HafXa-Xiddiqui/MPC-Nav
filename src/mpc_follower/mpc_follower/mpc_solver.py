...
n_controls = 2
n_obs = 3
safe_distance = 0.2        # Nearly touch the goal
obstacle_min_dist = 0.02     # HARD constraint: minimum distance to obstacles
...
for k in range(N):
    ...
    fade_heading = ca.fmax(0.0, 1.0 - dist_to_target / 0.2)
    ...
    vel_damping = ca.exp(-1.5 * dist_to_target)
    ...
    for i in range(n_obs):
        ...
        cost += 15.0 / (dist_sq + 1e-2)     # Soft penalty (softer than before)
        g.append(dist_sq - obstacle_min_dist**2)  # HARD constraint
...
ubx = np.tile(np.array([1.0, 2.0]), N)
